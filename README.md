...
**画像の説明**: この画像は、システムが実際に動作している様子を示しています。画面には以下の情報が表示されています：
...
**通常監視画面**
![通常監視画面](images/normal_monitoring.jpeg)
*QZSS衛星の受信状況とSDカードの状態を表示*

**DCX通報受信時**
![DCX通報受信](images/dcx_reception.jpeg)
*DCX通報を受信した際のリアルタイム表示*

**画像の説明**: この画像は、DCX通報（Disaster and Crisis Management Report Extended）を受信した際の画面です。黄色の背景で表示され、以下の情報が含まれています：
...
この画像により、DCX通報の詳細な解析結果が日本語で表示されていることが確認できます。

**DC Report通報受信時**
![DC Report通報受信](images/dc_report_reception.jpeg)
*DC Report通報を受信した際の詳細表示*

**画像の説明**: この画像は、DC Report通報（Disaster and Crisis Management Report）を受信した際の画面です。赤色の背景で表示され、以下の情報が含まれています：
...
この画像により、実際の災害通報の詳細内容が日本語で表示され、具体的な警報内容と対象地域が確認できます。

**通報詳細画面**
![通報詳細画面](images/report_detail.jpeg)
*保存された通報の詳細情報を表示*

**画像の説明**: この画像は、保存された通報の詳細情報を表示している画面です。黒い背景に白い文字で表示され、以下の情報が含まれています：
...
この画像により、保存された通報の詳細情報が整理されて表示され、過去の通報内容を確認できることが分かります。
...
![DC Report受信](images/dc_report_reception.jpeg)
*実際のDC Report受信画面：海上濃霧警報、海上風警報等の詳細表示*

**画像の説明**: この画像は、実際のDC Report通報を受信した際の画面です。赤色の背景で表示され、以下の情報が含まれています：
- **通報タイプ**: DC Report (Type 43)
- **受信時刻**: 09:20:31 JST（日本時間での正確な受信時刻）
- **衛星ID**: QZSS-7（QZSS衛星7号機からの信号）
- **メッセージ長**: 8 words
...
![DCX通報受信](images/dcx_reception.jpeg)
*実際のDCX通報受信画面：メッセージ・テスト等の詳細解析結果*

**画像の説明**: この画像は、実際のDCX通報を受信した際の画面です。黄色の背景で表示され、以下の情報が含まれています：
- **通報タイプ**: DCX Message (Type 44)
- **受信時刻**: 09:19:45 JST（日本時間での正確な受信時刻）
- **衛星ID**: QZSS-7（QZSS衛星7号機からの信号）
- **メッセージ長**: 8 words
...
#### **SDカードが認識されない場合（M5Stack Basic 2.7）**
1. **SDカードの確認**
